...
int main()
{
	...
	if (hFind != INVALID_HANDLE_VALUE) 
	{
		...
		do 
		{
			...
			//跳过标识
			FileDealing.SetFilePointer(8, WIN_FILE_MOVE_MODE_File_Current);

			//获取所引索引数量
			DWORD IndexSize;
			FileDealing.ReadFile(4, &IndexSize, nullptr);

			//跳过索引表
			FileDealing.SetFilePointer(IndexSize * 4, WIN_FILE_MOVE_MODE_File_Current);

			//获取代码大小
			DWORD CodeSize;
			FileDealing.ReadFile(4, &CodeSize, nullptr);

			//获取代码
			BYTE* Code = new BYTE[CodeSize];
			FileDealing.ReadFile(CodeSize, Code, nullptr);
			...
			//InI
			{
				...
				FileInI.WriteFile(4, &DialogSize, nullptr);
				FileInI.WriteFile(CodeSize, Code, nullptr);
				FileInI.CloseFile();
				delete[] Code;
			}
			...
			//获取剧本对话大小
			DWORD DialogSize;
			FileDealing.ReadFile(4, &DialogSize, nullptr);

			//跳过空白占位符号
			FileDealing.SetFilePointer(1, WIN_FILE_MOVE_MODE_File_Current);

			//读取剧本并关闭流对象
			BYTE* Dialogs = new BYTE[DialogSize];
			FileDealing.ReadFile(DialogSize-1, Dialogs, nullptr);
			...
			//Txt
			{
				...
				for (int i = 0; i < IndexSize - 1; i++)
				{
					...
					Str[Length] = '\n';
					FileTxT.WriteFile(Length + 1, Str, nullptr);
					delete[] Str;
					delete[] StrArray[i];
				}
				delete[] StrArray;
