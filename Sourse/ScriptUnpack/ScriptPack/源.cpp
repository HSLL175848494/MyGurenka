	...
	if (hFind != INVALID_HANDLE_VALUE)
	{
		...
		do
		{
			...
			//打开剧本txt以及其对应的ini文件读取数据
			{
				...
				FileTxT.Read(SizeTxT, DataTxT, nullptr);
				FileTxT.CloseFile();
				...
				FileInI.Read(SizeInI, DataInI, nullptr);
				FileInI.CloseFile();
			}
			...
			//写入标识
			FileSave.Write(8, (LPVOID)"ESCR1_00", nullptr);

			DWORD IndexSize = *(DWORD*)DataInI;

			//写入索引数目
			FileSave.Write(4, &IndexSize, nullptr);
			...
			//写入索引
			FileSave.Write(IndexSize*4, IndexArray, nullptr);

			//写入脚本大小
			SizeInI -= 4;
			FileSave.Write(4, &SizeInI, nullptr);

			//写入脚本
			FileSave.Write(SizeInI, DataInI + 4, nullptr);

			//写入剧本大小
			SizeTxT += 1;
			FileSave.Write(4, &SizeTxT, nullptr);

			//填充空白
			BYTE Fill = 0;
			FileSave.Write(1, &Fill, nullptr);

			//写入剧本
			FileSave.Write(SizeTxT-1, DataTxT, nullptr);
